...
class StatusBadge(tk.Label):
    ...
    def set(self,status):
        value=str(getattr(status,"value",status) or "unknown").upper();self.configure(text=value,bg=status_color(value),fg="#071018" if value not in {"BLOCKED","MISSING","RED"} else "white")
...
def tree(parent,columns,widths=None):
    ...
    for status,color in ((s,status_color(s)) for s in ("ready","partial","missing","blocked","unknown","not_applicable","green","yellow","red")):view.tag_configure(status,foreground=color)
    return view

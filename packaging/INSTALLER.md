...
ARX retains its portable PyInstaller distribution and adds an optional Inno Setup 6 installer for Windows 10/11 x64.
...
`build-installer.ps1` locates `ISCC.exe` from `PATH` or the standard Inno Setup 6 installation directories. Use `-IsccPath` for another installation. Output remains under `release/`, alongside an updated SHA-256 checksum file.

...
ARX 4.0.0 Beta 4 retains the complete portable PyInstaller distribution and its optional Inno Setup 6 or 7 installer for Windows 10/11 x64.

This installer is distinct from the existing `arx-prescanner` Python package. Python developers can use `python -m pip install arx-prescanner==4.0.0b4` after a separately authorized index publication to install the `arx` and `arx-desktop` entry points into a managed Python environment. The installer instead deploys the self-contained portable Windows payload and does not require a separately managed Python installation.
...
The release outputs are `arx_prescanner-4.0.0b4-py3-none-any.whl`, `arx_prescanner-4.0.0b4.tar.gz`, `ARX-Desktop-win-x64-v4.0.0-b4.zip`, `ARX-Desktop-Setup-win-x64-v4.0.0-b4.exe`, and `SHA256SUMS.txt`. They are isolated under `release/v4.0.0-b4/`; historical release directories are not overwritten. `build-installer.ps1` locates `ISCC.exe` from `PATH` or standard per-user and Program Files locations for Inno Setup 7 and 6. Use `-IsccPath` for another installation. Generated binaries and checksums are not committed.

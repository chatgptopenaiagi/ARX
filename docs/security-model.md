...
Phase C keeps GENERAL CHAT free of ARX context and makes ARX EVIDENCE CHAT an explicit attach/preview/consent flow. Machine DNA, Software DNA, Project DNA, deterministic conclusions, contradictions, unknowns, and supporting evidence are separately allowlisted and budgeted. OpenAI and Codex conversation histories are independent, bounded, and memory-only. Ask Both preserves two flat provider outcomes; Compare Responses is a separate explicit presentation action and can expose only textual overlap, differences, and unresolved statements. Neither operation can create consensus evidence, upgrade provenance, or mutate deterministic objects.

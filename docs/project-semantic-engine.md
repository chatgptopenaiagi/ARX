...
Inspected files are never executed. Oversized, malformed, undecodable, missing, unsupported, or symbolic-link inputs remain explicit evidence or unknowns. `RequirementEvidence` retains capability, semantic evidence type, source kind/path/key, bounded raw value, confidence, and provenance. Confidence is an uncalibrated detector-author weight used to retain relative branch strength; it is not a probability, measured accuracy, or statistical confidence. It cannot grant a selection record authority to overwrite a requirement record or upgrade provenance. Requirements retain their source manifest and field. Requirement edges distinguish `requires`, `selects`, and future causal dependencies. The complete current assignment inventory is in [Confidence semantics and assignment audit](confidence-semantics.md).
...
The ARX application version and schema version evolve independently. ARX 2.0.0, ARX 3.0.0rc1, ARX 4.0.0b1, and ARX 4.0.0b2 retained legacy machine/software commands and their schema `0.1` envelopes; ARX 4.0.0b3 and ARX 4.0.0b4 retain that compatibility. Project-aware AI output continues to use schema `0.2` and separates facts, decisions, selected providers, blockers, warnings, recommendations, policy constraints, unknowns, and evidence references. Phase C's UI-only comparison objects and Local AI operational state are not added to that contract. See [AI contract 0.2](ai-contract-0.2.md).

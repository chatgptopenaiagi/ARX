...
.\scripts\collect-windows-lifecycle-evidence.ps1 `
  ...
  -ArtifactPath .\ARX-Desktop-Setup-win-x64-v4.0.0-b4.exe `
  -ExpectedVersion 4.0.0b4 `
...
| Upgrade | Install the Beta 4 candidate over the approved prior test build using the stable AppId | One product registration remains; expected files and versions update; ARX still launches non-elevated |
